...
checkpoint = torch.load("outputs/best_model_0.773.pth", map_location=DEVICE)
...
def inference_on_image(orig_image: np.ndarray, resize_dim=None, threshold=0.25):
    ...
    for box, label_idx in zip(boxes, labels):
        ...
        color = COLORS[label_idx % len(COLORS)]  # BGR color [::-1]
        cv2.rectangle(orig_image, (box[0], box[1]), (box[2], box[3]), color, 2)
        cv2.putText(orig_image, class_name, (box[0], box[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        cv2.putText(
            orig_image,
            fps_text,
            (int((w_orig / 2) - 50), 30),
